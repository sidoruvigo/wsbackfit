...
!     *********************************************************
!     *********************************************************     
     subroutine sback(X,Y,W,n,npar,kbin,h,M,muhat,family,X0,M0,muhat0,n0,B) 
     !!DEC$ ATTRIBUTES DLLEXPORT::sback
     !!DEC$ ATTRIBUTES C, REFERENCE, ALIAS:'sback_' :: sback
     implicit none
     ...
     do it=1,itmax     !@@@@@@@@@@ Bucle de Iteraciones @@@@@@
          ...
          do i=1,n
               do j = 1,npar
                   Y2(i) = Z2(i) - M(i,j)  !Eliminamos componente no paramétrica
               end do
          end do
          
          call RegL(X,Y2,Wz,n,npar,B,Predl)
          do i=1,n
               Y2(i) = Z2(i) - Predl(i)     !Eliminamos componente paramétrica
                do j = 1, npar
                    Ml(i,j) = B(j+1)*X(i,j)
               end do
          end do
