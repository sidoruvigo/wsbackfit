...
!     *********************************************************     
     ...
     double precision eps,media,s0,X(n,npar),Y(n),W(n),M(n,npar),h(npar),family,cte,&
     linc,muhat(n),devian,devold,p0,sumw, X0(n0,npar),M0(n0,npar),muhat0(n0),B(npar+1)
     double precision,external::slinc,diriv,weight,dev
     double precision,allocatable::h0(:), Z2(:), Wz(:), Eta(:),EtaL(:), EtaS(:), & 
     eta0(:), Y2(:)
     allocate (h0(npar),Z2(n),Wz(n),Eta(n),EtaL(n),EtaS(n),eta0(n0),Y2(n))
     ...
     do it=1,itmax     !@@@@@@@@@@ Bucle de Iteraciones @@@@@@
          ...
          Y2=Z2-EtaS  ! Eliminamos la parte no param?trica          
          call RegL(X,Y2,Wz,n,npar,B,Etal)
...
1   continue
     do i=1,n0  ! Predicci?n 
         Eta0(i)=Eta0(i)+B(1)      
         do j=1,npar
             Eta0(i)=Eta0(i)+ B(j+1)*X0(i,j)
             M0(i,j)=M0(i,j)+ B(j+1)*X0(i,j)
		 end do
     end do
     call Linv(n0,Eta0,Muhat0,linc) 
  
     h=h0
     deallocate (h0,Z2,Wz,Eta,EtaL,EtaS,eta0,Y2)
 end
!     *********************************************************
!     *********************************************************     
...
!     *********************************************************
!     *********************************************************     
     subroutine Vcoef(X,Z,Y,W,n,npar,Zl,nparl,kbin,h,M,Mx,muhat,family,X0,Z0,Z0l,M0,Mx0,muhat0,n0,B) 
     !!DEC$ ATTRIBUTES DLLEXPORT::vcoef
     !!DEC$ ATTRIBUTES C, REFERENCE, ALIAS:'vcoef_' :: vcoef
