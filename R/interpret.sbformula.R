interpret.sbformula <- function(formula, method = "sback") {

    env <- environment(formula)
    if(inherits(formula, "character"))
        formula <- stats::as.formula(formula)
    tf <- stats::terms.formula(formula, specials = c("sb"))
    ...
    II <- ifelse(length(II), matrix(unlist(II), nrow = 2), matrix(0, nrow = 2))
    # II <- if(length(II)) {
